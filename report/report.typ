...
$
To see more fine-grained differences between different privacy budgets $epsilon$, we report the validation _perplexity_ instead, expressed as
$
...
In @tab:perplexity, note that the validation perplexities across the two studies are not comparable, as this metric is highly dependent on the validation data. However, as a general trend, we see that DP models perform worse than non-DP models in both studies, corroborating the plot shown in @fig:utility. One discrepancy between the studies is that #cite(<yu>, form: "prose") obtained a lower percent increase from baselines, averaging to 26.1%, while ours are 40.3% and 39.0% for comparable privacy budgets $epsilon = 4, 8$ in this study. This difference can be explained by various factors, the most prominent being that their training dataset consists of 42K samples, which is larger than ours, and so the models might have learned to generalize better even with DP.
