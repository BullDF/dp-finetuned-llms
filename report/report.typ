...
and fill in the blank with a 9-digit number. By varying the number of times the canaries were inserted and the privacy budget $epsilon$ in DP-SGD, the authors were able to quantify how much DP helps with reducing unintentional memorization of the training data.
...
With this canary form, we varied the number of times the canaries were inserted into the training set, with $"freq" = 1, 5, 10, 50$. We also pre-specified some values of privacy budget to conduct the experiment, with $epsilon = 0.5, 1, 2, 4, 8, infinity$, where $epsilon = infinity$ means vanilla gradient descent. We set $delta$ in the $(epsilon, delta)$-DP definition to be $1 slash n$ where $n = 10000$ is the size of our training data and is a standard practice in DP training #cite(<dwork>). For each pair of frequency and privacy budget $epsilon$, we fine-tuned GPT-2 Small using the training set with canaries inserted for 3 epochs. The Python library `Opacus` was used for DP-SGD support. Each model was then assessed by calculating the _log-perplexity_ on the validation set as a proxy for model utility, mathematically expressed as
$
  "log-perp"_theta (x_1, dots.c.h, x_n) = -1/n sum_(i=1)^n log bb(P)(x_i|f_theta (x_1, dots.c.h, x_(i-1))),
$
where $x_1, dots.c.h, x_n$ is an input sequence, $f$ denotes the model, and $theta$ is a set of model parameters. To see more fine-grained differences between different privacy budgets $epsilon$, we report the validation _perplexity_ instead of log-perplexity, expressed as
$
  "perp"_theta (x_1, dots.c.h, x_n) = exp("log-perp"_theta (x_1, dots.c.h, x_n)).
$
...
@fig:utility shows the line plot of validation perplexity vs privacy budget $epsilon = 0.5, 1, 2, 4, 8, infinity$ used in this project. From the plot, we see that for all values of $epsilon$, the effect from the frequency of canaries inserted into the training set is negligible, as the validation perplexities are mostly indistinguishable across frequencies. This is expected because the frequencies of canaries do not affect how much the model understands language during fine-tuning. Furthermore, we notice that the higher the privacy budget $epsilon$, the smaller the validation perplexity, which is also expected, as a higher privacy budget means less privacy and thus the model can learn more patterns from the training data. These results are further compared against those from #cite(<yu>, form: "prose") in @tab:perplexity.
...
In @tab:perplexity, note that the validation perplexities across the two studies are not comparable, as this metric is highly dependent on the validation data. However, as a general trend, we see that DP models perform worse than non-DP models in both studies, corroborating the plot shown in @fig:utility. One discrepancy between the studies is that #cite(<yu>, form: "prose") obtained a lower percent increase from baselines, averaging to 26.1%, while ours are 40.3% and 39.0% for comparable privacy budgets $epsilon = 4, 8$. This difference can be explained by various factors, the most prominent being that their training dataset consists of 42K samples, which is larger than ours, and so the models might have learned to generalize better even with DP.
