...
So why is DP-SGD useful? In the study by #cite(<carlini>, style: "apa", form: "prose"), the authors demonstrated that naive gradient descent has a high risk of leading neural networks to unintentionally memorize fine-grained information, such as social security numbers and oddly-specific details. They used a method called _canary insertion_, where they purposefully inserted formatted inputs such as "The random number is ...", known as canaries, into the training dataset. The results showed that unintentional memorization exists, and in their experiments, only differentially private training techniques were able to eliminate the issue completely.
