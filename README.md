...
|---|---|
| Proposal | |
| 1 — Data prep (`1_data.ipynb`) | `solomonk/reddit_mental_health_posts`, 10k train / 1k val, MAX_LENGTH=128 |
| 2 — Baseline training (`2_train.ipynb`) | ε = ∞, no canaries; perplexity = **23.76** |
| 3 — Canary insertion (`3_canaries.ipynb`) | 4 datasets, one per frequency (1, 5, 10, 50×), seed=2412 |
| 4 — DP-SGD sweep (`4_dp_train.ipynb`) | 24 runs; perplexity ε=0.5→34.1, ε=8→33.0, ε=∞→24.8 |
| 5 — Evaluation (`5_evaluate.ipynb`) | Canary exposure scoring + plots |
| 6 — Ablation (`6_ablation.ipynb`) | Base-rate check for freq=10 anomaly |
